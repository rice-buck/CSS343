...
LList<Object>::LList( ) {
  header = new LListNode<Object>;                 // 0th node is a dummy.
  header->next = nullptr;
  header->item = Object{};
}
...
LList<Object>::LList( const LList &rhs ) {
  header = new LListNode<Object>;                 // create a dummy
  header->next = nullptr;   
  header->item = Object{};                        // then assign rhs to this.
  *this = rhs;
}
...
// Clear the list by repeatedly finding and removing the first object until the list is empty. But the dummy header remains.
template<class Object>
void LList<Object>::clear() {
    while (header->next != nullptr) {
        LListNode<Object>* temp = header->next;
...
const LList<Object> &LList<Object>::operator=( const LList &rhs ) {
  if ( this != &rhs ) { // avoid self-assignment
    ...
    int index=0;
    for (LListNode<Object> *rnode = rhs.header->next; 
      rnode != nullptr;
